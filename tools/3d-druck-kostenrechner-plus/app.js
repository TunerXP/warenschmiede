...
const DEFAULT_CALCULATION_PROFILES = Object.freeze([
  Object.freeze({id:'standard',name:'Standard Verkauf',marginPercent:35,laborMinutes:10,hourlyRate:50,failRate:5}),
  Object.freeze({id:'hobby',name:'Selbstkosten / Hobby',marginPercent:10,laborMinutes:5,hourlyRate:25,failRate:3}),
  Object.freeze({id:'business',name:'Gewerblich sauber',marginPercent:60,laborMinutes:15,hourlyRate:60,failRate:8}),
  Object.freeze({id:'express',name:'Express / Einzelstück',marginPercent:85,laborMinutes:20,hourlyRate:70,failRate:10})
]);
const cloneDefaultProfiles = () => DEFAULT_CALCULATION_PROFILES.map(profile => ({...profile}));

function app(){ return {
  appVersion:'1.9.5',
  settingsOpen:false, settingsNotice:'', positionNoticeText:'', positionNoticeJobId:'', highlightedProfileId:'', historyOpen:false, preflightOpen:false, historyNotice:'', historySearch:'', historyDateFilter:'all', historySort:'newest', activeTab:'firma', tabs:[{id:'firma',label:'Firma'},{id:'design',label:'Logo & Design'},{id:'zahlung',label:'Zahlung'},{id:'steuer',label:'Steuer'},{id:'presets',label:'Presets'},{id:'qr',label:'QR-Code'},{id:'ausgabe',label:'Ausgabe'},{id:'daten',label:'Daten'}],
  ...
  applySnapshot(data){ if(!data) return; const keepHistory=this.history; const keepOpen=this.historyOpen; const keepNotice=this.historyNotice; Object.assign(this, data); if(!this.output) this.output={compactPdf:false,compactForceArticle:false,showStorageInfo:true}; if(this.output.compactForceArticle===undefined) this.output.compactForceArticle=false; this.ensureBuiltInProfiles(); this.history=keepHistory; this.historyOpen=keepOpen; this.historyNotice=keepNotice; },
  seed(){ this.presets.materials=[{id:'pla',name:'PLA Standard',priceKg:20,co2:2.5},{id:'pla_plus',name:'PLA+',priceKg:25,co2:2.6},{id:'petg',name:'PETG',priceKg:25,co2:3.5},{id:'asa',name:'ASA',priceKg:35,co2:4.2},{id:'tpu',name:'TPU',priceKg:38,co2:3.2},{id:'custom',name:'Eigenes Material',priceKg:25,co2:0}]; this.presets.printers=[{id:'a1',name:'Bambu Lab A1 / ähnlich',watts:180,machineHourly:1.5,loadFactor:70},{id:'x1c',name:'Bambu Lab X1C / ähnlich',watts:250,machineHourly:2.5,loadFactor:75},{id:'custom',name:'Eigener Drucker',watts:200,machineHourly:2,loadFactor:70}]; this.presets.profiles=cloneDefaultProfiles(); this.ensureBuiltInProfiles(); },
  ensureBuiltInProfiles(){ if(!this.presets) this.presets={materials:[],printers:[],profiles:[]}; if(!Array.isArray(this.presets.profiles)) this.presets.profiles=[]; for(const profile of DEFAULT_CALCULATION_PROFILES){ if(!this.presets.profiles.some(existing=>existing.id===profile.id)) this.presets.profiles.push({...profile}); } },
  isBuiltInProfile(profileOrId){ const id=typeof profileOrId==='object' ? profileOrId?.id : profileOrId; return DEFAULT_CALCULATION_PROFILES.some(profile=>profile.id===id); },
  defaultProfileFor(profileOrId){ const id=typeof profileOrId==='object' ? profileOrId?.id : profileOrId; return DEFAULT_CALCULATION_PROFILES.find(profile=>profile.id===id); },
  isProfileModified(profile){ const standard=this.defaultProfileFor(profile); if(!standard) return false; return profile.name!==standard.name || ['marginPercent','laborMinutes','hourlyRate','failRate'].some(key=>Number(profile[key])!==Number(standard[key])); },
  profileStatus(profile){ return this.isBuiltInProfile(profile) ? (this.isProfileModified(profile)?'Angepasst':'Standard') : 'Eigenes Profil'; },
  showSettingsNotice(message){ this.settingsNotice=message; clearTimeout(this._settingsNoticeTimer); this._settingsNoticeTimer=setTimeout(()=>{ this.settingsNotice=''; },4000); },
  showPositionNotice(job,message){ this.positionNoticeJobId=job?.id||''; this.positionNoticeText=message; clearTimeout(this._positionNoticeTimer); this._positionNoticeTimer=setTimeout(()=>{ this.positionNoticeText=''; this.positionNoticeJobId=''; },4000); },
  resetProfile(profile){ const standard=this.defaultProfileFor(profile); if(!standard) return; if(!this.isProfileModified(profile)){ this.showSettingsNotice('Dieses Profil verwendet bereits die Standardwerte.'); return; } if(!confirm(`Profil „${standard.name}“ auf die Warenschmiede-Standardwerte zurücksetzen?`)) return; Object.assign(profile,{...standard}); this.showSettingsNotice(`Profil „${standard.name}“ wurde zurückgesetzt.`); },
  removeProfile(profile){ if(this.isBuiltInProfile(profile)) return; const fallback=this.presets.profiles.find(candidate=>candidate.id!==profile.id)?.id || 'standard'; this.presets.profiles=this.presets.profiles.filter(candidate=>candidate.id!==profile.id); this.jobs.forEach(job=>{ if(job.profileId===profile.id) job.profileId=fallback; }); },
  profileName(job){ return this.presets.profiles.find(profile=>profile.id===job?.profileId)?.name || ''; },
  reapplyProfile(job){ const profile=this.presets.profiles.find(candidate=>candidate.id===job?.profileId); if(!profile){ this.showPositionNotice(job,'Das ausgewählte Profil wurde nicht gefunden.'); return; } this.applyProfile(job); this.showPositionNotice(job,'Profilwerte wurden in diese Position neu übernommen.'); },
  openProfileSettings(job){ this.settingsOpen=true; this.activeTab='presets'; const id=job?.profileId; this.$nextTick(()=>{ const element=id ? document.getElementById('cost-profile-'+id) : document.getElementById('calculation-profiles'); if(!element) return; element.scrollIntoView({behavior:window.matchMedia?.('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'center'}); this.highlightedProfileId=id||''; element.querySelector('input')?.focus({preventScroll:true}); setTimeout(()=>{ if(this.highlightedProfileId===id) this.highlightedProfileId=''; },2600); }); },
