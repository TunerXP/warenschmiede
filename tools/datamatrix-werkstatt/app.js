...
(function (root) {
  ...
  const api={generateSeries,parseManualList,sanitizeFilename,calculateA4Layout,calculateIntegerScale,buildBwipOptions,createLabelModel,createLabelMarkup,paginateLabels,clampSheetPage,validateLabelLayout,migrateProject,validateInputs,validateVersion,validateProject,DEFAULT_INPUTS,MAX_VERSIONS,PROJECT_SCHEMA,PROJECT_VERSION,LOCAL_DRAFT_KEY};
  ...
  function printSheet(){if(!render())return;try{const l=updateSheet();if(!l)throw new Error('Druckbogen passt nicht auf A4.');const inputs=currentInputs(),pages=paginateLabels(state.values,l),w=root.open('','_blank');if(!w)throw new Error('Druckfenster wurde blockiert.');const labels=page=>page.map(v=>createLabelMarkup(createLabelModel(v,inputs,svg(v)),{cutLines:inputs.cutLines})).join('');const sheets=pages.map(page=>`<main class="print-page" style="${sheetCss(l)}">${labels(page)}</main>`).join('');w.document.write(`<!doctype html><html><head><title>Data-Matrix-Druckbogen</title><style>@page{size:A4 ${inputs.orientation};margin:0}*{box-sizing:border-box}body{margin:0}.print-page{width:calc(var(--page-w)*1mm);height:calc(var(--page-h)*1mm);padding:calc(var(--margin)*1mm);display:grid;grid-template-columns:repeat(var(--columns),calc(var(--label-w)*1mm));grid-template-rows:repeat(var(--rows),calc(var(--label-h)*1mm));gap:calc(var(--gap-y)*1mm) calc(var(--gap-x)*1mm);break-after:page;page-break-after:always;background:#fff}.print-page:last-child{break-after:auto;page-break-after:auto}.sheet-label{position:relative;overflow:hidden;padding:1.5mm;display:flex;align-items:center;justify-content:center;background:var(--code-bg)}.label-code{flex:1;min-width:0;min-height:0;display:grid;place-items:center}.label-code svg{display:block;width:auto;height:auto;max-width:100%;max-height:100%;aspect-ratio:1}.label-info{flex:none;min-width:0;max-width:100%;display:flex;flex-direction:column;gap:.6mm;overflow-wrap:anywhere}.layout-code-text,.layout-info-portrait{flex-direction:column}.layout-info-landscape{display:grid;grid-template-columns:minmax(0,45%) minmax(0,1fr);gap:1.5mm}.layout-code-only .label-code{width:100%;height:100%}.layout-info-portrait .label-title{flex:none}.border-fine{box-shadow:inset 0 0 0 .25mm var(--label-border)}.border-strong{box-shadow:inset 0 0 0 .6mm var(--label-border)}.corners-rounded{border-radius:2mm}.cut-lines{outline:.2mm dashed #777;outline-offset:-.2mm}.text-small{font:7pt system-ui}.text-normal{font:9pt system-ui}.text-large{font:11pt system-ui}.align-center{text-align:center}@media print{body{print-color-adjust:exact}}</style></head><body>${sheets}<script>onload=()=>setTimeout(()=>print(),100)<\/script></body></html>`);w.document.close();message('Druckdialog vorbereitet. Bitte echte Druckgröße und Scanner testen.');}catch(e){message(e.message||'Drucken fehlgeschlagen.','error');}}
